...
from pdfminer.high_level import extract_text as pdf_extract_text  # if this import name fails, switch back to pdfminer.high_level
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# BILL TEXT FILE OUTPUT (NEW): where to store bill text .txt files
...
# ---- NEW: safe filename & writer ----
def _safe_filename(name: str) -> str:
    name = (name or "").strip()
    name = re.sub(r"[^A-Za-z0-9._-]+", "_", name)
    return (name or "unknown") + ("" if name.endswith(".txt") else ".txt")

def write_bill_text_file(ga: int, billno: str, bill_text: str) -> Path:
    """
    Save bill text to bill_texts/GA<ga>/<billno>.txt and return the path.
    Creates directories as needed. Overwrites existing file.
    """
    target_dir = BILL_TEXT_ROOT / f"GA{ga}"
    target_dir.mkdir(parents=True, exist_ok=True)
    fn = _safe_filename(billno)
    p = target_dir / fn
    p.write_text(bill_text or "", encoding="utf-8")
    return p
...
class Row:
    ...
    def to_csv_row(self) -> Dict[str, str]:
        return {
            ...
            "bill text": self.bill_text,  # now a file path
...
# ---- Bill processing ----
def process_bill(billno: str, url: str, ga: int) -> Optional[Row]:
    """
    ...
    Saves bill text to bill_texts/GA<ga>/<billno>.txt and sets CSV field to that path.
    """
    try:
        r = fetch(url)
    except Exception:
    ...
    # Bill text used for matching & enrichment (not stored in CSV directly anymore)
    ...
    if not hits:
        # Save an empty file anyway? We stick to prior behavior: only keep matched.
        return None

    # ===== NEW: write bill text to file and use file path in CSV =====
    path_to_text = write_bill_text_file(ga=ga, billno=billno, bill_text=bill_text)
    ...
    # CSV 'bill text' now stores the relative path to the .txt file
    row.bill_text = str(path_to_text.as_posix())
...
def main():
    ...
    for ga in range(GA_START, GA_END + 1):
        ...
        for billno, url, _ in tqdm(links, desc=f"GA {ga}", leave=False):
            ...
            if key in seen_bills:
                continue
            processed += 1
            row = process_bill(billno, url, ga)  # pass GA for file path placement
            if row:
                seen_bills.add(key)
    ...
    print(f"Bill texts saved under: {BILL_TEXT_ROOT.resolve()}")
