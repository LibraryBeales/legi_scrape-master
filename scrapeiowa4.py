...
# -*- coding: utf-8 -*-
"""
Iowa Legislature bill scraper — GA 91 (or range), keyword filter,
LGE/LGI attachment-first text, iframe fallback, Bill History parsing,
sponsor extraction, and **bill text saved to .txt files**.

Changes requested:
- Save bill text into bill_texts/GA{ga}/{billno}.txt and put that PATH in the CSV "bill text" column.
- Remove "Policy name" column entirely.
- Parse dates (Introduced/Passed/Effective/Enacted/Dead) directly from the Bill History widget:
  <div class="billAction"><table class="billActionTable"> ... </table></div>
...
BILL_TEXT_ROOT = Path("bill_texts")  # root dir for saved bill texts
# ===============================================================
...
# --- Introduced date helpers (also used by history parser) ---
DATE_PATS = [
...
def extract_introduced_from_history(billbook_html: str) -> Optional[str]:
    """
    (Fallback) Find 'Introduced' in the page if our main parser misses it.
    """
    soup = soupify(billbook_html)

    containers = []
    containers.extend(soup.select("#billHistory, .billHistory, #history, .history, div.billAction"))
...
# ===================== BILL HISTORY PARSER (revised for billAction widget) =====================
def _find_bill_history_table(soup: BeautifulSoup) -> Optional[BeautifulSoup]:
    # Prefer the explicit history widget you showed
    t = soup.select_one("div.billAction table.billActionTable")
    if t:
        return t
    # header → next table
    hdr = soup.find(lambda tag: tag.name in ("h1","h2","h3","h4")
                    and "bill history" in tag.get_text(" ").strip().lower())
    if hdr:
        t2 = hdr.find_next("table")
        if t2:
            return t2
    # fallback: any table that looks like actions
    for t3 in soup.find_all("table"):
        txt = clean(t3.get_text(" "))
        if any(k in txt for k in ("Introduced", "Passed House", "Passed Senate",
                                  "Signed by Governor", "Approved by Governor", "Effective")):
            return t3
    return None

# Pull the date for a row: first <td> if present; else YYYYMMDD from any link in the row
def _extract_row_date(tr: BeautifulSoup) -> str:
    tds = tr.find_all("td")
    if tds:
        first_cell = clean(tds[0].get_text(" "))
        if re.match(r"\d{1,2}/\d{1,2}/\d{4}\b", first_cell):
            return first_cell
    # Fallback: date embedded in hrefs (e.g., /HJNL/20250304_... or ?hdate=20250304)
    for a in tr.find_all("a", href=True):
        href = a["href"]
        m = re.search(r"(?:[?&](?:hdate|date|actionDate)=|\D)(\d{8})(?!\d)", href, flags=re.I)
        if m:
            return _ymd_to_mdy(m.group(1))
    return ""

# Regexes for action matching
...
EFFECTIVE  = re.compile(r"\bEffective(?:\s+(?:on|date))?\b", re.I)
...
def parse_bill_history_fields(billbook_html: str, billno: str) -> Dict[str, str]:
    ...
    # Iterate history rows
    for tr in (table.select("tbody tr") or table.find_all("tr")):
        tds = tr.find_all("td")
        if not tds:
            continue
        # Action text is everything after the first cell
        action_text = clean(" ".join(td.get_text(" ") for td in tds[1:])) if len(tds) > 1 else clean(tr.get_text(" "))
        date = _extract_row_date(tr)
        ...
        if m2 and not out["Dead (Y/N)"]:
            ...
            out["Chamber where died"] = (m2.group(1).title() if m2.group(1) else introduced_chamber)
    ...
    out["Pending (Y/N)"] = "N" if out["Enacted (Y/N)"] == "Y" or out["Dead (Y/N)"] == "Y" else "Y"
    return out
...
class Row:
    ...
    Link_to_bill: str = ""
    bill_text: str = ""  # this will now be a PATH to the saved .txt
    Cosponsor: str = ""
    Introduced_YN: str = ""
    Introduced_date: str = ""
...
# ---- Helpers for saving bill text ----
def _sanitize_filename(name: str) -> str:
    return re.sub(r"[^A-Za-z0-9._-]+", "_", name)

def save_bill_text(ga: int, billno: str, text: str) -> str:
    ga_dir = BILL_TEXT_ROOT / f"GA{ga}"
    ga_dir.mkdir(parents=True, exist_ok=True)
    filename = _sanitize_filename(billno) + ".txt"
    path = ga_dir / filename
    with open(path, "w", encoding="utf-8") as f:
        f.write(text)
    return str(path).replace("\\", "/")  # make CSV path OS-agnostic
...
def process_bill(ga: int, billno: str, url: str) -> Optional[Row]:
    ...
    save bill text to /bill_texts/GA{ga}/{billno}.txt and put that path in CSV.
    """
    try:
        r = fetch(url)
    except Exception:
    ...
    # Bill text used for matching & saving
    bill_text = (combined_attach_text or iframe_text or "").strip()
    if not bill_text:
        bill_text = ""

    # Extract primary sponsor from the LG text (prefer attachment-primary)
    ...
    # Parse Bill History (dates, passes, enacted, effective, cosponsors, dead)
    status = parse_bill_history_fields(billbook_html, billno)

    # Fallback: if parser didn’t find an Introduced date, pull it from generic history scan
    if not status["Introduced date"]:
        intro = extract_introduced_from_history(billbook_html)
        if intro:
    ...
    # Save bill text to disk and record path
    ...
    # Put PATH (not text) in CSV
    row.bill_text = text_path
